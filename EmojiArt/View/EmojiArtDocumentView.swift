...
struct EmojiArtDocumentView: View {
    ...
    var body: some View {
        VStack {
            ...
            GeometryReader { geometry in
                ZStack {
                    Color.white.overlay(
                        OptionalImage(uiImage: self.document.backgroudImage)
    ...
    private func zoomGesture() -> some Gesture {
        MagnificationGesture()
            .updating($gestureZoomScale) { lastestGestrueScale, gestureZoomScale, transcation in
                gestureZoomScale = lastestGestrueScale
            }
    ...
    private func panGesture() -> some Gesture {
        DragGesture()
            .updating($gesturePanOffset) { lastestDragGestureValue, gesturePanOffset, transcation in
    ...
    private func doubleTapToZoom(in size: CGSize) -> some Gesture {
        TapGesture(count: 2)
            .onEnded { _ in
                withAnimation {
                    self.zoomToFit(self.document.backgroudImage, in: size)
                }
        }
    }
